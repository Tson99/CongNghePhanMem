...
def align_image(image, coordinate_dict):
    if len(coordinate_dict) < 3:
        # cv2.imshow('Error: Cant detect >= 3 corners', image_detect_corner)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        raise ValueError('Cant detect >= 3 corner')

    coordinate_dict = get_center_point(coordinate_dict)  # convert (xmin, ymin, xmax, ymax) to (x_center, y_center)
...
def non_max_suppression_fast(boxes, labels, overlapThresh):
    ...
    idxs = np.argsort(y2)  # X : Horizontal, Y : Vertical
...
def sort_text_front(detection_boxes, detection_labels):
    ...
    img_boxes = detection_boxes[detection_labels == 6]  #ADD
    ...
    img_boxes = sort_each_category(img_boxes)

    return id_boxes, name_boxes, birth_boxes, home_boxes, add_boxes, img_boxes #ADD


def sort_text_back(detection_boxes, detection_labels):
    detection_labels = np.array(detection_labels)
    nation_boxes = detection_boxes[detection_labels == 1]
    religion_boxes = detection_boxes[detection_labels == 2]
    identifying_characteristics_boxes = detection_boxes[detection_labels == 3]
    registration_day_boxes = detection_boxes[detection_labels == 4]
    place_of_registration_boxes = detection_boxes[detection_labels == 5]

    nation_boxes = sort_each_category(nation_boxes)
    religion_boxes = sort_each_category(religion_boxes)
    identifying_characteristics_boxes = sort_each_category(identifying_characteristics_boxes)
    registration_day_boxes = sort_each_category(registration_day_boxes)
    place_of_registration_boxes = sort_each_category(place_of_registration_boxes)

    return nation_boxes, religion_boxes, identifying_characteristics_boxes, registration_day_boxes, place_of_registration_boxes #ADD
...
def limited_cropped_image(cropped_image):
    if cropped_image[3] > 500:
        cropped_image[3] = 500
    if cropped_image[2] > 300:
        cropped_image[2] = 300
    if cropped_image[1] < 0:
        cropped_image[1] = 0
...
def sort_each_category(category_text_boxes):
    ...
    for line in merged_text_boxes:
        limited_cropped_image(line)
